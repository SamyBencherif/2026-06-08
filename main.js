...
//   ctx.fillRect(0,0,width,height);
  ...
  //Physics
    ...
    angularVelocity += 100*HAxis*dt;
    angularVelocity = clamp(-7,angularVelocity,7);
    angularVelocity -= sign(angularVelocity)*30*dt;
    ...
    velocity.y += 1000*Math.sin(angle-Math.PI/2)*VAxis*dt;
    velocity.x += 1000*Math.cos(angle-Math.PI/2)*VAxis*dt;
    if (abs(velocity)>300)
    {
    ...
    // wall boundaries for the mod
    if (position.x + playerRadius > innerWidth/2) velocity.x = -Math.abs(velocity.x);
    if (position.x - playerRadius < -innerWidth/2) velocity.x = Math.abs(velocity.x);
    if (position.y + playerRadius > innerHeight/2) velocity.y = -Math.abs(velocity.y);
    if (position.y - playerRadius < -innerHeight/2) velocity.y = Math.abs(velocity.y);
    ...
    ctx.strokeStyle = `hsl(${time()/10}, 100%, 50%)`;
   ...
   if (auto && time() - timeSinceLastInput > 300)
   {
    HAxis = [-1, 0, 1][~~(Math.random()*3)];
    VAxis = [-1, 0, 1][~~(Math.random()*3)];
...
function onKey(event, keycode, down)
{
    ...
    {
        HAxis = down*((keycode==39 || keycode==68)-(keycode==37 || keycode==65))
    }
